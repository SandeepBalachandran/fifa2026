'use client';

import { useState, useEffect } from 'react';
import type { Fixture } from '@/lib/football-data/types';

export interface TeamMatchesState {
  data: Fixture[] | null;
  loading: boolean;
  error: string | null;
}

const cache = new Map<string, Fixture[]>();

export function useTeamMatches(teamId: string | null): TeamMatchesState & { retry: () => void } {
  const [state, setState] = useState<TeamMatchesState>({
    data: null,
    loading: false,
    error: null,
  });
  const [retryCount, setRetryCount] = useState(0);

  useEffect(() => {
    if (!teamId) {
      setState({ data: null, loading: false, error: null });
      return;
    }

    if (cache.has(teamId)) {
      setState({ data: cache.get(teamId)!, loading: false, error: null });
      return;
    }

    let cancelled = false;
    setState({ data: null, loading: true, error: null });

    fetch(`/api/teams/${teamId}/matches`)
      .then((res) => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        return res.json() as Promise<Fixture[]>;
      })
      .then((data) => {
        if (!cancelled) {
          cache.set(teamId, data);
          setState({ data, loading: false, error: null });
        }
      })
      .catch((err) => {
        if (!cancelled) setState({ data: null, loading: false, error: String(err) });
      });

    return () => { cancelled = true; };
  }, [teamId, retryCount]);

  const retry = () => {
    if (teamId) cache.delete(teamId);
    setRetryCount((c) => c + 1);
  };

  return { ...state, retry };
}
